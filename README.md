...
- 独立“主题讨论”模式支持圆桌、头脑风暴、正反辩论和专家会诊：2～6 个差异化角色共享上下文，Leader 轮末主持，轮次/消息/Token 三重预算阻止无限对话；讨论可暂停、恢复、定向追问、生成 Markdown 产物并一键转为正式任务。
...
纯思考场景可从“主题讨论”进入，不要求 Workspace。创建讨论时设置主题、背景、讨论模式、参与角色和预算；角色回复使用固定背景、共享记忆与最近对话，不会每轮无限复制完整 Transcript。讨论结束后可由 Leader 生成总结、行动计划、Design Brief、PRD 或决策矩阵，也可以选择 Workspace 将结论转换为现有 Evidence 驱动任务。
