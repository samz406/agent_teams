...
export class EvidenceService {
  derive(
        ...
        status:
          command.exitCode === null
            ...
            : command.exitCode === 0
              ? "PASS"
              : "FAIL",
        detail: command.output.slice(-4000),
      });
      if (isTestCommand(command.command))
        evidence.push({
          type: "TEST",
          title: command.command,
          status:
            command.exitCode === 0
              ...
              : command.exitCode === null
                ? "UNVERIFIED"
                : "FAIL",
          detail: command.output.slice(-8000),
        });
    }
    if (!evidence.some((item) => item.type === "TEST")) {
      const combined = `${stdout}\n${stderr}`;
      const proof = combined.match(
        /(?:Tests?|Test Files?)\s*[:：]?\s*(?:\d+\s+)?(?:passed|PASS|通过)[\s\S]{0,500}/i,
      );
      if (proof)
        evidence.push({
          type: "TEST",
          title: "Runtime reported test result",
          status: exitCode === 0 ? "PASS" : "UNVERIFIED",
          detail: proof[0],
        });
    }
    ...
    if (usage) {
      ...
      evidence.push({
        type: "USAGE",
        title: `${formatTokens(usage.usage.totalTokens)} tokens · ${cost}`,
        status: "PASS",
  ...
  private extractCommands(
    output: string,
  ): Array<{ command: string; exitCode: number | null; output: string }> {
    const results: Array<{
      command: string;
      exitCode: number | null;
      output: string;
    ...
    for (const line of output.split(/\r?\n/)) {
      try {
        ...
        /* plain text is retained as transcript, not promoted to command evidence */
      }
    }
  ...
  private walk(
    ...
    for (const child of Object.values(record)) {
      if (Array.isArray(child))
        child.forEach((item) => this.walk(item, results));
...
const isTestCommand = (command: string): boolean =>
  testPatterns.some((pattern) => pattern.test(command));
const looksLikeCommand = (value: string): boolean =>
  value.length < 2000 && /(?:\s|\/|\\|-)/.test(value);
...
const dedupe = <T extends { command: string }>(values: T[]): T[] => [
  ...new Map(values.map((value) => [value.command, value])).values(),
];
