...
export class AdapterConversationExecutor implements ConversationExecutor {
  ...
  async execute(
    ...
    await this.queue.enqueue(
      ...
      async () => {
        ...
        try {
          ...
          const launch =
            participant.nativeSessionId && adapter.supportsNativeResume
              ? await adapter.resume(input)
              : await adapter.start(input);
          this.active.set(conversation.id, {
            queueId,
            child: launch.child,
            adapter,
          });
          ...
          if (code !== 0)
            throw new Error(
              `Agent 对话执行失败（exit=${code ?? "null"}）：${stderr.slice(-1000)}`,
            );
          const parsed = adapter.parse(stdout);
          const reported = extractUsageSummary(stdout);
          const fallback = extractTokenUsage(
            stdout,
            prompt,
          ...
          result = {
            ...
            inputTokens: reported?.usage.inputTokens ?? fallback.inputTokens,
            outputTokens: reported?.usage.outputTokens ?? fallback.outputTokens,
            cachedInputTokens: reported?.usage.cachedInputTokens ?? 0,
            cacheCreationInputTokens:
              reported?.usage.cacheCreationInputTokens ?? 0,
            reasoningOutputTokens: reported?.usage.reasoningOutputTokens ?? 0,
            totalTokens:
              reported?.usage.totalTokens ??
              fallback.inputTokens + fallback.outputTokens,
    ...
    this.queued.delete(conversation.id);
    if (this.cancelled.delete(conversation.id))
      throw new Error("讨论已暂停或结束");
  ...
  async cancel(conversationId: string, force: boolean): Promise<void> {
    ...
    if (queued && this.queue.cancel(queued)) {
      this.queued.delete(conversationId);
      return;
    }
    if (active)
      await (force
        ? active.adapter.cancel(active.child)
        : active.adapter.interrupt(active.child));
  }

  async shutdown(): Promise<void> {
    await Promise.all(
      [...new Set([...this.active.keys(), ...this.queued.keys()])].map((id) =>
        this.cancel(id, true),
      ),
    );
  }
...
export class ConversationEngine {
  ...
  start(conversationId: string): void {
    ...
    if (conversation.status === "READY_TO_SUMMARIZE")
      throw new Error("讨论轮次已结束，请生成总结或转为任务");
    this.db.updateConversationStatus(conversationId, "RUNNING");
    this.changed();
    if (!this.loops.has(conversationId)) {
      this.loops.add(conversationId);
      void this.runLoop(conversationId).finally(() =>
  ...
  async pause(conversationId: string): Promise<void> {
    ...
    if (conversation.status !== "RUNNING")
      throw new Error("讨论当前不在运行中");
    this.db.updateConversationStatus(conversationId, "PAUSED");
    this.db.interruptActiveConversationRound(conversationId);
    await this.executor.cancel(conversationId, false);
    this.changed();
  }
  ...
  async end(conversationId: string): Promise<void> {
    ...
    this.db.interruptActiveConversationRound(conversationId);
    await this.executor.cancel(conversationId, true);
    this.systemTurn(
      conversationId,
      "讨论已结束。你可以生成总结、行动计划、设计 Brief、PRD、决策矩阵、战略议题清单或六帽分析报告，也可以将结论转为正式任务。",
    );
    this.changed();
  }
  ...
  extend(conversationId: string, additionalRounds: number): void {
    const conversation = this.requireConversation(conversationId);
    if (conversation.status !== "READY_TO_SUMMARIZE")
      throw new Error("只有已停止的讨论可以追加轮次");
    const rounds = Math.min(3, Math.max(1, Math.round(additionalRounds)));
    if (conversation.currentRound >= 50)
      throw new Error("讨论已达到最高 50 轮");
  ...
  sendMessage(
    ...
    if (
      ...
      !this.db
        ...
        .some((item) => item.id === targetParticipantId)
    )
      throw new Error("目标角色不属于当前讨论");
    this.db.createConversationTurn({
      conversationId,
      roundId: null,
    ...
    this.db.updateConversationProgress(
      conversationId,
    ...
    if (memory)
      this.db.updateConversationMemory(conversationId, {
        ...memory,
        userPreferences: dedupe([
  ...
  async summarize(
    ...
    const turn = this.db.createConversationTurn({
      ...
      agentId: agent.id,
      speakerType: "leader",
      speakerName: leader.roleName,
      content: "",
      status: "QUEUED",
    });
    this.changed();
    try {
      this.db.updateConversationTurn(turn.id, { status: "RUNNING" });
      this.changed();
      ...
      this.db.createConversationDeliverable(
        conversationId,
        type,
        `${conversation.title} · ${deliverableLabel(type)}`,
        result.content,
      );
  ...
  exportMarkdown(conversationId: string): { title: string; content: string } {
    ...
    const transcript = this.db
      ...
      .map(
        (turn) =>
          `### ${turn.speakerName}\n\n${turn.content || `_${turn.status}${turn.error ? `：${turn.error}` : ""}_`}`,
      )
      .join("\n\n");
    return {
      title: `${conversation.title}.md`,
  ...
  private async runLoop(conversationId: string): Promise<void> {
    try {
      while (this.db.getConversation(conversationId)?.status === "RUNNING") {
        const conversation = this.requireConversation(conversationId);
        if (this.limitReached(conversation)) {
          this.readyToSummarize(conversation);
          return;
        }
        const roundNumber = conversation.currentRound + 1;
        const focus = this.nextFocus(conversation);
        const round = this.db.createConversationRound(
          conversationId,
          roundNumber,
          focus,
        );
        const participants =
          this.db.getConversationParticipants(conversationId);
        const latestTarget = [...this.db.getConversationTurns(conversationId)]
          .reverse()
          .find(
            (item) => item.speakerType === "human" && item.participantId,
          )?.participantId;
        const ordered = [
          ...participants
            ...
            .sort(
              (a, b) =>
                Number(b.id === latestTarget) - Number(a.id === latestTarget),
            ),
          ...participants.filter((item) => item.isLeader),
        ];
        let successes = 0;
        ...
        const latest = this.requireConversation(conversationId);
        this.db.finishConversationRound(
          round.id,
        ...
        if (!successes && latest.status === "RUNNING") {
          ...
          this.systemTurn(
            conversationId,
            "本轮没有 Agent 成功返回，请检查 Runtime 配置后恢复讨论。",
          );
          this.changed();
          return;
        }
        ...
        if (this.limitReached(progressed)) {
          this.readyToSummarize(progressed);
          return;
        }
      }
    } catch (error) {
      ...
      if (current?.status === "RUNNING") {
        ...
        this.systemTurn(
          conversationId,
          `讨论编排失败：${error instanceof Error ? error.message : String(error)}`,
        );
        this.changed();
      }
    }
  ...
  private async runParticipant(
    ...
    try {
      ...
      this.changed();
      const memoryVersion =
        this.db.getConversationMemory(conversation.id)?.version ??
        participant.memoryVersion;
      const prompt = this.participantPrompt(
        conversation,
        participant,
        roundNumber,
        focus,
      ...
      const cancelled =
        latest?.status === "PAUSED" || latest?.status === "READY_TO_SUMMARIZE";
      this.db.updateConversationTurn(turn.id, {
        status: cancelled ? "CANCELLED" : "FAILED",
        error: error instanceof Error ? error.message : String(error),
      });
      if (!cancelled)
        this.db.updateConversationProgress(conversation.id, roundNumber, 1, 0);
      this.changed();
      return false;
    }
  }

  private participantPrompt(
    conversation: Conversation,
    participant: ConversationParticipant,
    roundNumber: number,
    focus: string,
    ...
    const newTurns = this.db
      .getConversationTurnsAfter(
        conversation.id,
        participant.lastSeenTurnSequence,
      )
      .filter((item) => item.status === "COMPLETED" && item.content);
    const updates = newTurns
      .map(
        (item) =>
          `[消息 #${item.sequence} · ${item.speakerName}${item.participantId === participant.id && item.speakerType === "human" ? " → 请你回答" : ""}] ${item.content.slice(0, 1600)}`,
      )
      .join("\n\n");
  ...
  private summaryPrompt(
    ...
    const memory = this.db.getConversationMemory(conversation.id);
    const turns = this.db
      .getConversationTurns(conversation.id)
      .filter((item) => item.status === "COMPLETED" && item.content)
      .slice(-40)
      .map((item) => `[${item.speakerName}] ${item.content.slice(0, 1800)}`)
      .join("\n\n");
  ...
  private refreshMemory(conversationId: string, roundId: string): void {
    ...
    const turns = this.db
      .getConversationTurns(conversationId)
      .filter(
        (item) => item.roundId === roundId && item.status === "COMPLETED",
      );
    const notes = turns.map(
      (item) =>
        `${item.speakerName}：${item.content.replace(/\s+/g, " ").slice(0, 360)}`,
    );
    const lines = turns.flatMap((item) =>
      item.content
        .split(/\r?\n/)
        .map((line) => line.replace(/^[-*#\s]+/, "").trim())
        .filter(Boolean),
    );
    const consensus = dedupe([
      ...memory.consensus,
      ...lines.filter((line) => /共识|同意|赞同|一致|可以确定/.test(line)),
    ]).slice(-12);
    const disagreements = dedupe([
      ...memory.disagreements,
      ...lines.filter((line) => /反对|不同意|分歧|风险|但是|然而/.test(line)),
    ]).slice(-12);
    const openQuestions = dedupe([
      ...memory.openQuestions,
      ...lines.filter((line) => /[？?]$/.test(line)),
    ]).slice(-12);
    const summary = [memory.summary, ...notes]
      .filter(Boolean)
      .join("\n")
      .slice(-6000);
    this.db.updateConversationMemory(conversationId, {
  ...
  private nextFocus(conversation: Conversation): string {
    ...
    if (conversation.mode === "retreat") {
      const progress =
        conversation.currentRound / Math.max(1, conversation.maxRounds);
      const stage =
        progress < 0.25
      ...
      return memory?.openQuestions[0]
        ? `${stage}；同时回应待确认问题：${memory.openQuestions[0]}`
        : stage;
    }
    if (conversation.mode === "six-hats") {
      ...
      return memory?.openQuestions[0]
        ? `${stage}；同时处理待确认问题：${memory.openQuestions[0]}`
        : stage;
    }
    if (memory?.openQuestions.length) return memory.openQuestions[0];
    if (conversation.currentRound === 0)
      return "独立给出你的核心判断、依据和一个具体例子";
    if (conversation.currentRound === 1)
      return "回应其他角色，指出你认同、质疑或需要补充的部分";
  ...
  private systemTurn(conversationId: string, content: string): void {
    this.db.createConversationTurn({
      conversationId,
      roundId: null,
      participantId: null,
...
const modeInstructions: Record<Conversation["mode"], string> = {
  ...
  debate:
    "正反辩论：主动暴露假设、反例、代价和二阶影响，以论证而不是立场取胜。",
  consultation: "专家会诊：给出假设、依据、风险、建议以及仍需确认的信息。",
  retreat:
    "务虚会：拉开时间尺度，先观察变化与反思现状，暴露路径依赖和关键假设；避免过早讨论具体执行，最终收敛为战略议题与待验证方向。",
  "six-hats":
    "六顶思考帽：蓝帽管理流程，白帽处理事实，红帽表达直觉，黑帽审查风险，黄帽寻找价值，绿帽创造选项；各角色不得越过自己的思考边界。",
};
const deliverableLabel = (type: ConversationDeliverable["type"]): string =>
  ({
    SUMMARY: "讨论总结",
    ACTION_PLAN: "行动计划",
    ...
    STRATEGIC_AGENDA: "战略议题清单",
    SIX_HATS_REPORT: "六帽分析报告",
    MARKDOWN: "主题文档",
  })[type];
const estimateTokens = (value: string): number =>
  Math.max(1, Math.ceil(value.length / 3));
const totalTokens = (result: ChatResult): number =>
