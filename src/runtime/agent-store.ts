...
export function updateAgentRecord(
  ...
  try {
    const current = db
      .prepare("SELECT * FROM t_agent WHERE id=?")
      .get(input.id) as Record<string, unknown> | undefined;
    if (!current) throw new Error("Agent 不存在");
    if (!input.name.trim() || !input.responsibility.trim())
      throw new Error("Agent 名称和责任边界不能为空");
    db.prepare(
      `UPDATE t_agent SET
    ...
    db.prepare("INSERT INTO t_event VALUES (?,?,?,?,?,?)").run(
      randomUUID(),
      "agent",
      input.id,
      "AGENT_UPDATED",
    ...
    return {
      ...
      currentRunId:
        current.current_run_id === null ? null : String(current.current_run_id),
