...
export class RuntimeClient {
  ...
  private ensureProcess(): UtilityProcess {
    ...
    const child = utilityProcess.fork(join(__dirname, "runtime.js"), [], {
      serviceName: "Moxt Agent Runtime",
      env: { ...process.env, MOXT_DATA_DIR: this.dataDirectory },
      stdio: "pipe",
    });
    child.on("message", (message) =>
      this.handleMessage(message as RuntimeProcessMessage),
    );
    child.on("exit", (code) => {
    ...
    child.stdout?.on("data", (value) =>
      console.info(`[runtime] ${String(value).trimEnd()}`),
    );
    child.stderr?.on("data", (value) =>
      console.error(`[runtime] ${String(value).trimEnd()}`),
    );
    this.child = child;
    return child;
  }
