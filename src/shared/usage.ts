...
const PRICING: Pricing[] = [
  { pattern: /gpt-5\.6[-_ ]?sol/i, provider: 'openai', input: 4, cachedInput: 0.4, output: 20, version: 'openai-codex-2026-07-30' },
  { pattern: /gpt-5\.6[-_ ]?terra/i, provider: 'openai', input: 2, cachedInput: 0.2, output: 12, version: 'openai-codex-2026-07-30' },
  { pattern: /gpt-5\.6[-_ ]?luna/i, provider: 'openai', input: 0.2, cachedInput: 0.02, output: 1.2, version: 'openai-codex-2026-07-30' },
  { pattern: /gpt-5\.5(?!.*rosalind)/i, provider: 'openai', input: 5, cachedInput: 0.5, output: 30, version: 'openai-codex-2026-07-30' },
  { pattern: /gpt-5\.3[-_ ]?codex/i, provider: 'openai', input: 1.75, cachedInput: 0.175, output: 14, version: 'openai-codex-2026-07-30' },
  { pattern: /gpt-5\.2/i, provider: 'openai', input: 1.75, cachedInput: 0.175, output: 14, version: 'openai-codex-2026-07-30' },
  { pattern: /claude[-_ ]?sonnet[-_ ]?5/i, provider: 'anthropic', input: 2, cachedInput: 0.2, cacheCreation: 2.5, output: 10, version: 'anthropic-2026-09-01' },
  { pattern: /claude[-_ ]?opus[-_ ]?5/i, provider: 'anthropic', input: 5, cachedInput: 0.5, cacheCreation: 6.25, output: 25, version: 'anthropic-2026-09-01' },
  { pattern: /claude[-_ ]?opus[-_ ]?4[-_. ]?8/i, provider: 'anthropic', input: 5, cachedInput: 0.5, cacheCreation: 6.25, output: 25, version: 'anthropic-2026-09-01' },
  { pattern: /claude[-_ ]?fable[-_ ]?5/i, provider: 'anthropic', input: 10, cachedInput: 1, cacheCreation: 12.5, output: 50, version: 'anthropic-2026-09-01' }
]
...
export function extractUsageSummary(output: string): UsageSummary | null {
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    try {
      ...
      walkJson(value, record => {
        const model = firstString(record.model, record.model_name, record.modelName)
        if (model) models.push(model)
        const cost = firstNumber(record.total_cost_usd, record.totalCostUsd, record.cost_usd, record.costUSD)
        if (cost !== null && cost >= 0) costs.push(cost)

        const usageRecord = asRecord(record.usage) ?? asRecord(record.token_usage) ?? asRecord(record.tokenUsage)
        if (usageRecord) {
    ...
    } catch { /* Runtime transcripts can contain plain text alongside JSONL. */ }
  }

  if (!candidates.length) return null
  const selected = candidates.sort((a, b) => rawTokenScore(b.usage) - rawTokenScore(a.usage))[0]
  ...
  if (reportedCost !== null && reportedCost > 0) {
    return { schemaVersion: 1, provider, model, usage, costUsd: roundCost(reportedCost), costType: 'REPORTED', pricingVersion: 'runtime-reported' }
  }
  const estimate = estimateCost(model, usage)
  return { schemaVersion: 1, provider: estimate?.provider ?? provider, model, usage, costUsd: estimate?.costUsd ?? null, costType: estimate ? 'ESTIMATED' : 'UNAVAILABLE', pricingVersion: estimate?.version ?? null }
}

export function parseUsageEvidence(detail: string): UsageSummary | null {
  try {
    const value = JSON.parse(detail) as UsageSummary
    if (value?.schemaVersion !== 1 || !value.usage || typeof value.usage.totalTokens !== 'number') return null
    return value
  } catch { return null }
}

export function usageForRun(run: Run, liveOutput?: string): UsageSummary | null {
  if (liveOutput && ['QUEUED', 'STARTING', 'RUNNING'].includes(run.status)) {
    const live = extractUsageSummary(liveOutput)
    if (live) return live
  }
  const evidence = [...run.evidence].reverse().find(item => item.type === 'USAGE')
  return evidence ? parseUsageEvidence(evidence.detail) : null
}

export function aggregateRunUsage(runs: Run[], live: Record<string, string> = {}): UsageAggregate {
  const aggregate: UsageAggregate = { usage: emptyUsage(), costUsd: 0, pricedRuns: 0, usageRuns: 0, unpricedRuns: 0 }
  for (const run of runs) {
    ...
    aggregate.usage.cacheCreationInputTokens += summary.usage.cacheCreationInputTokens
    aggregate.usage.reasoningOutputTokens += summary.usage.reasoningOutputTokens
    aggregate.usage.totalTokens += summary.usage.totalTokens
    if (summary.costUsd !== null) { aggregate.costUsd += summary.costUsd; aggregate.pricedRuns += 1 } else aggregate.unpricedRuns += 1
  }
...
export function formatDuration(run: Pick<Run, 'startedAt' | 'endedAt'>): string | null {
  ...
  return minutes < 60 ? `${minutes}m${rest ? `${rest}s` : ''}` : `${Math.floor(minutes / 60)}h${minutes % 60}m`
}

function estimateCost(model: string | null, usage: TokenUsage): { costUsd: number; version: string; provider: UsageSummary['provider'] } | null {
  ...
  if (pricing.provider === 'openai') {
    ...
    cost = (uncached * pricing.input + usage.cachedInputTokens * pricing.cachedInput + usage.outputTokens * pricing.output) / million
  } else {
    cost = (usage.inputTokens * pricing.input + usage.cachedInputTokens * pricing.cachedInput + usage.cacheCreationInputTokens * (pricing.cacheCreation ?? pricing.input) + usage.outputTokens * pricing.output) / million
  }
...
function normalizeUsage(record: Record<string, unknown>): PartialUsage {
  return {
    ...
    cachedInputTokens: firstNumber(record.cached_input_tokens, record.cachedInputTokens, record.cache_read_input_tokens, record.cacheReadInputTokens) ?? 0,
    cacheCreationInputTokens: firstNumber(record.cache_creation_input_tokens, record.cacheCreationInputTokens, record.cache_write_input_tokens, record.cacheWriteInputTokens) ?? 0,
    reasoningOutputTokens: firstNumber(record.reasoning_output_tokens, record.reasoningOutputTokens, asRecord(record.output_tokens_details)?.reasoning_tokens) ?? 0,
    totalTokens: firstNumber(record.total_tokens, record.totalTokens) ?? undefined
  }
}
...
function finalizeUsage(raw: PartialUsage, provider: UsageSummary['provider']): TokenUsage {
  ...
  return { inputTokens: raw.inputTokens, outputTokens: raw.outputTokens, cachedInputTokens: raw.cachedInputTokens, cacheCreationInputTokens: raw.cacheCreationInputTokens, reasoningOutputTokens: raw.reasoningOutputTokens, totalTokens }
}
...
function detectProvider(value: unknown): UsageSummary['provider'] {
  ...
  if (record.total_cost_usd !== undefined || record.modelUsage !== undefined || record.model_usage !== undefined) return 'anthropic'
  return 'unknown'
}

function inferProviderFromModel(model: string | null): UsageSummary['provider'] {
...
function mostSpecificModel(models: string[]): string | null { return models.filter(Boolean).sort((a, b) => b.length - a.length)[0] ?? null }
function roundCost(value: number): number { return Math.round(value * 1_000_000) / 1_000_000 }
function trim(value: number): string { return value >= 100 ? value.toFixed(0) : value >= 10 ? value.toFixed(1).replace(/\.0$/, '') : value.toFixed(2).replace(/0+$/, '').replace(/\.$/, '') }
