...
> = {
  "cross-project": {
    name: "跨项目协同开发",
    description: "多个项目围绕契约并行调查、开发与联调",
  },
  incident: {
    name: "线上问题会诊",
    description: "并行收集证据，区分症状、假设与根因",
  },
  "bug-fix": {
    name: "Bug 修复",
    description: "先复现、再修复，并由独立 Agent 验证",
  },
  refactor: {
    name: "大型重构",
    description: "建立行为基线后分步、可回退地重构",
  },
  release: {
    name: "发布前检查",
    description: "并行检查风险，基于证据做 Go / No-Go",
  },
};
...
function p(
  ...
  deliverable: string,
  exitCriteria: string[],
  humanMode: PhaseTemplate["humanMode"],
): PhaseTemplate {
...
export function phasesFor(
  ...
  return WORKFLOWS[type].map((phase, index) => ({
    ...phase,
...
export function canAdvance(
  ...
  if (phase.humanMode === "IN_LOOP" && !hasApprovedArtifact)
    return { ok: false, reason: "当前阶段是人工 Gate，必须先批准 Artifact" };
  return { ok: true };
}
