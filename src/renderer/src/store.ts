...
export const useAppStore = create<AppState>((set) => ({
  ...
  notice: null,
  load: async () =>
    set({ snapshot: await window.moxt.getSnapshot(), ready: true }),
  apply: (event) => {
    ...
    if (event.type === "run.activity")
      set((state) => ({
        live: {
          ...state.live,
          [event.runId]: (state.live[event.runId] || "") + event.chunk,
        },
      }));
    if (event.type === "runtime.notice")
      set({
        notice: {
          type: event.level === "error" ? "error" : "success",
          text: event.message,
        },
      });
  },
  notify: (type, text) => {
    set({ notice: { type, text } });
    window.setTimeout(() => set({ notice: null }), 3500);
  },
}));
