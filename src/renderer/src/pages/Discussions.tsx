...
import type { ConversationMode, ConversationStatus, CreateConversationInput } from '../../../shared/contracts'
...
function NewConversation({ onClose, onCreated, notify }: { onClose(): void; onCreated(id: string): void; notify(type: 'success' | 'error', text: string): void }): import('react').JSX.Element {
  ...
  const [form, setForm] = useState<Omit<CreateConversationInput, 'participants'>>({ title: '', topic: '', background: '', mode: 'roundtable', maxRounds: 20, maxMessages: 1000, maxTokens: 1_000_000 })
