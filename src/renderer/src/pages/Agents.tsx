...
export default function Agents({
  ...
  async function save(): Promise<void> {
    const payload = {
      name: form.name.trim(),
      icon: (form.name.trim() || editing?.name || "A")
        .slice(0, 1)
        .toUpperCase(),
      description: form.description.trim(),
      responsibility: form.responsibility.trim(),
      qualityBar: form.quality
        .split("\n")
        .map((item) => item.trim())
        .filter(Boolean),
      runtime: form.runtime,
      command: form.command.trim() || null,
      argsTemplate: editing?.argsTemplate ?? null,
      workspaceIds: editing?.workspaceIds ?? [],
      permissions: {
        read: form.read,
        write: form.write,
        shell: form.shell,
    ...
    try {
      if (editing)
        await window.moxt.updateAgent({ id: editing.id, ...payload });
      else await window.moxt.createAgent(payload);
      setOpen(false);
      notify(
        "success",
        editing
          ? "Agent 配置已更新，下一个 Run 开始生效"
          : "Agent 已加入 Agent Pool",
      );
    } catch (error) {
  ...
  return (
    <section className="page">
      ...
      <div className="agent-table editable-agent-table">
        ...
        {snapshot.agents.map((agent) => (
          <div className="table-row agent-row" key={agent.id}>
            <span>
              <i className="agent-avatar">{agent.icon}</i>
              <b>
                {agent.name}
            ...
            <span>
              <b className={`status ${agent.status.toLowerCase()}`}>
                {statusLabel(agent.status)}
              </b>
            </span>
            <span>
              <button className="table-action" onClick={() => edit(agent)}>
                <Edit3 />
                编辑
              </button>
            </span>
      ...
      {open && (
        <div className="modal-backdrop">
          <div className="modal">
            <header>
              <div>
                <h2>{editing ? `编辑 ${editing.name}` : "创建自定义 Agent"}</h2>
                <p>
                  {editing
                    ? "修改长期角色配置；正在运行的 Run 不受影响，新配置从后续 Run 生效。"
                    : "先定义责任和质量标准，再选择 Runtime。"}
                </p>
              </div>
            ...
            <div className="form-grid">
              ...
              <label>
                ...
                <select
                  value={form.runtime}
                  onChange={(e) =>
                    setForm({ ...form, runtime: e.target.value as RuntimeType })
                  }
                >
              ...
              <label className="full">
                ...
                <input
                  ...
                  placeholder="留空使用检测到的 Runtime 路径"
                />
              </label>
              <div className="permission-options full">
                ...
                <label>
                  <input
                    ...
                    checked={form.write}
                    onChange={(e) =>
                      setForm({ ...form, write: e.target.checked })
                    }
                  />
                  Write
                </label>
                <label>
                  <input
                    ...
                    checked={form.shell}
                    onChange={(e) =>
                      setForm({ ...form, shell: e.target.checked })
                    }
                  />
                  Shell
                </label>
                <label>
                  <input
                    ...
                    checked={form.git}
                    onChange={(e) =>
                      setForm({ ...form, git: e.target.checked })
                    }
                  />
                  Git
                </label>
            ...
            <div className="permission-note">
              <ShieldCheck />
              这里是 Agent 默认权限；创建任务时仍会通过 Agent-Workspace Binding
              做最终硬校验，不会因为编辑 Agent 自动放宽已有任务权限。
            </div>
            <footer>
