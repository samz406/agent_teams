...
import { aggregateRunUsage, formatCost, formatDuration, formatTokens, usageForRun } from '../../../shared/usage'
...
export default function TaskRoom({ change }: { change: Change }): import('react').JSX.Element {
  ...
  async function kick(): Promise<void> {
    try { await window.moxt.startChange(change.id, 'User requested start / continue from task room'); notify('success', '已检查当前阶段并启动/继续执行') }
    catch (error) { notify('error', errorText(error)) }
    setMenuOpen(false)
  }
  ...
  return <section className="task-room">
    <header className="room-header"><div><div className="eyebrow">任务 #{change.number} · {WORKFLOW_LABELS[change.workflowType].name}</div><h1>{change.title}</h1><p>{change.description}</p></div><div className="room-actions"><div className="usage-meter"><button className="usage-pill" onClick={() => setUsageOpen(value => !value)}><strong>{taskUsage.usageRuns ? formatTokens(taskUsage.usage.totalTokens) : '—'}</strong><span>tokens</span><em>{taskUsage.usageRuns ? formatCost(taskUsage) : '计量中'}</em></button>{usageOpen && <UsagePopover runs={runs} tasks={tasks} live={live} agents={team}/>}</div><span className="running-dot">● {change.status}</span><div className="task-menu"><button className="icon-btn" aria-label="任务操作" onClick={() => setMenuOpen(value => !value)}><MoreHorizontal/></button>{menuOpen && <div className="task-menu-popover"><button onClick={() => void kick()}><Play/>启动 / 继续执行</button><button onClick={() => { setTab('workflow'); setMenuOpen(false) }}><Activity/>查看 Workflow</button><button onClick={() => void refresh()}><RefreshCw/>刷新任务状态</button><button onClick={() => void replan()}><Play/>让 Leader 重新规划</button><button onClick={() => void controlAll('pause')}><Pause/>暂停全部运行</button><button className="danger" onClick={() => void controlAll('stop')}><CircleStop/>终止全部运行</button></div>}</div></div></header>
...
function UsagePopover({ runs, tasks, live, agents }: { runs: Run[]; tasks: Task[]; live: Record<string, string>; agents: Agent[] }): import('react').JSX.Element {
  const total = aggregateRunUsage(runs, live)
  const byAgent = agents.map(agent => ({ agent, usage: aggregateRunUsage(runs.filter(run => run.agentId === agent.id), live) })).filter(item => item.usage.usageRuns)
  const wasteRuns = runs.filter(run => Boolean(run.retryReason) || ['FAILED', 'CANCELLED', 'INTERRUPTED'].includes(run.status) || (run.taskId ? tasks.find(task => task.id === run.taskId)?.status === 'REWORK' : false))
  const waste = aggregateRunUsage(wasteRuns, live)
  return <div className="usage-popover"><header><div><strong>本任务资源消耗</strong><small>来自 Runtime Usage，不使用 tokenizer 猜测</small></div><span>{runs.length} Runs</span></header><div className="usage-total"><div><span>Total Tokens</span><strong>{total.usageRuns ? total.usage.totalTokens.toLocaleString() : '—'}</strong></div><div><span>Estimated Cost</span><strong>{formatCost(total)}</strong></div></div><div className="usage-grid"><div><span>Input</span><strong>{formatTokens(total.usage.inputTokens)}</strong></div><div><span>Output</span><strong>{formatTokens(total.usage.outputTokens)}</strong></div><div><span>Cache</span><strong>{formatTokens(total.usage.cachedInputTokens + total.usage.cacheCreationInputTokens)}</strong></div><div><span>Reasoning</span><strong>{formatTokens(total.usage.reasoningOutputTokens)}</strong></div></div>{byAgent.length > 0 && <><h4>By Agent</h4><div className="usage-agent-list">{byAgent.map(item => <div key={item.agent.id}><span><i className="agent-avatar">{item.agent.icon}</i>{item.agent.name}</span><strong>{formatTokens(item.usage.usage.totalTokens)}</strong><em>{formatCost(item.usage)}</em></div>)}</div></>}{waste.usageRuns > 0 && <div className="usage-waste"><span>Retry / Failed 消耗</span><strong>{formatTokens(waste.usage.totalTokens)} · {formatCost(waste)}</strong></div>}<p className="usage-note">金额为 CLI 报告值或按对应模型公开费率估算，不代表 Plus / Pro / 企业订阅的实际账单扣费。未知模型只统计 Token，不猜价格。</p></div>
}
...
function AgentInspector({ agent, runs, live, onClose }: { agent: Agent; runs: Run[]; live: Record<string, string>; onClose(): void }): import('react').JSX.Element {
  ...
  const session = snapshot.agentSessions.find(item => item.id === run?.agentSessionId)
  ...
  return <aside className="inspector"><header><div className="agent-avatar">{agent.icon}</div><div><h2>{agent.name}</h2><p>{agent.runtime} · {agent.status}</p></div><button onClick={onClose}><X/></button></header><div className="inspector-actions"><button onClick={() => void control('pause')} disabled={!run || run.status !== 'RUNNING'}><Pause/>暂停</button><button onClick={() => void control('resume')} disabled={!run || !['PAUSED','INTERRUPTED'].includes(run.status)}><Play/>继续</button><button onClick={() => void control('stop')} disabled={!run || !['RUNNING','STARTING','QUEUED'].includes(run.status)}><CircleStop/>终止</button><button onClick={() => void control('retry')} disabled={!run}><RefreshCw/>重试</button></div><div className="session-layout"><div className="session-list"><h4>Session / Run</h4>{runs.map(item => { const itemUsage = usageForRun(item, live[item.id]); return <button className={item.id === run?.id ? 'active' : ''} onClick={() => setSelected(item.id)} key={item.id}><span>{item.id.slice(0, 8)}</span><small>{item.status} · {itemUsage ? `${formatTokens(itemUsage.usage.totalTokens)} tokens` : item.startedAt ? new Date(item.startedAt).toLocaleString() : '排队中'}</small></button> })}</div>{run ? <div className="run-detail"><div className="run-meta"><span><Clock3/> {run.status}{duration ? ` · ${duration}` : ''}</span><span><Terminal/> {run.executable}</span>{usage && <span className="run-usage-meta">{formatTokens(usage.usage.totalTokens)} tokens · {formatSingleCost(usage.costUsd)}{usage.model ? ` · ${usage.model}` : ''}</span>}</div>{session && <div className="session-facts"><span>Agent Session</span><strong>{session.id.slice(0, 8)} · Native {session.nativeSessionId || '等待 Runtime 返回'}</strong></div>}{task && <div className="session-facts"><span>Task</span><strong>{task.status} · {task.phaseId}</strong></div>}<h4>实时 Transcript</h4><pre>{(live[run.id] || run.stdout || run.stderr || '暂无输出').slice(-12000)}</pre><h4>Evidence</h4><div className="evidence-list">{run.evidence.map(item => <div key={item.id}><span className={`evidence-status ${item.status.toLowerCase()}`}>{item.status}</span><strong>{item.title}</strong><p>{item.type === 'USAGE' ? 'Runtime Token Usage / Cost Evidence' : item.detail.slice(0, 600)}</p></div>)}</div></div> : <div className="empty"><Terminal/><h3>尚无 Run</h3><p>在 Team Chat 中给这个 Agent 发送任务。</p></div>}</div></aside>
}
...
function WorkflowView({ change }: { change: Change }): import('react').JSX.Element {
  ...
  const active = snapshot.runs.filter(r => r.changeId === change.id && ['QUEUED', 'STARTING', 'RUNNING'].includes(r.status))
  ...
  const phaseUsage = (phaseId: string) => { const ids = new Set(allTasks.filter(task => task.phaseId === phaseId).map(task => task.id)); return aggregateRunUsage(snapshot.runs.filter(run => run.changeId === change.id && Boolean(run.taskId) && ids.has(run.taskId!)), live) }
  async function advance(): Promise<void> { try { await window.moxt.advanceWorkflow(change.id); notify('success', 'Workflow 已进入下一阶段并尝试自动执行') } catch (error) { notify('error', errorText(error)) } }
  async function start(): Promise<void> { try { await window.moxt.startChange(change.id, 'User requested start / continue from Workflow view'); notify('success', '已检查当前阶段并启动/继续执行') } catch (error) { notify('error', errorText(error)) } }
  ...
  return <div className="workflow-view"><div className="phase-list">{phases.map((phase, i) => { const usage = phaseUsage(phase.id); return <div className={phase.status.toLowerCase()} key={phase.id}><i>{phase.status === 'DONE' ? <Check/> : i + 1}</i><div><strong>{phase.name}</strong><p>{phase.goal}</p>{usage.usageRuns > 0 && <small className="phase-usage">{formatTokens(usage.usage.totalTokens)} tokens · {formatCost(usage)}</small>}</div><span>{phase.status}</span></div> })}</div><div className="phase-detail"><span className="chip">当前阶段</span><h2>{current.name}</h2><p>{current.goal}</p><h4>当前 Task</h4><div className="task-state-list">{tasks.length ? tasks.map(task => <div key={task.id}><span className={`status ${task.status.toLowerCase()}`}>{task.status}</span><strong>{task.title}</strong><small>Required: {task.requiredEvidence.join(' · ')}</small></div>) : <p>Scheduler 会自动按当前 Workflow 阶段选择 Agent 并建立 Task。</p>}</div><h4>Blocking Issues</h4><div className="issue-list">{issues.length ? issues.map(issue => <div key={issue.id}><span>{issue.severity}</span><strong>{issue.title}</strong><p>{issue.description}</p><button onClick={() => void resolveIssue(issue.id)}>标记已解决</button></div>) : <p>当前没有未解决的 Blocking Issue。</p>}</div><h4>交付物</h4><div className="deliverable"><FileText/>{current.deliverable}</div><h4>Exit Criteria</h4>{current.exitCriteria.map(item => <label key={item}><input type="checkbox" readOnly checked={tasks.length > 0 && tasks.every(task => task.status === 'ACCEPTED')}/>{item}</label>)}<h4>Human Mode</h4><span className="human-mode">{current.humanMode}</span><h4>Active Runs</h4><p>{active.length ? `${active.length} 个 Agent 正在执行` : '当前没有运行中的 Agent'}</p>{!active.length && change.status !== 'DONE' && <button className="primary" onClick={() => void start()}><Play/>启动 / 继续执行</button>}<button className="primary" onClick={() => void advance()}>由状态机校验并推进<ArrowRight/></button></div></div>
}
