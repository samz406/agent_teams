...
function WorkflowView({ change }: { change: Change }): import('react').JSX.Element {
  ...
  const tasks = snapshot.tasks.filter(task => task.changeId === change.id && task.phaseId === current.id)
