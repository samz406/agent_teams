...
export default function ConversationRoom({ conversation, onBack, onOpenTask }: Props): import('react').JSX.Element {
  ...
  return <section className="conversation-room">
    ...
    {tab === 'discussion'
      ...
      : <ResultPanel conversation={conversation} deliverables={deliverables} participants={[...new Set(participants.map(item => item.agentId))]} busy={busy} onSummarize={summarize} onOpenTask={onOpenTask}/>}
  </section>
}
