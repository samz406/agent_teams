      ...
      ? <div className="conversation-layout">
          ...
          <main className="conversation-feed">
            ...
            <div className="conversation-scroll">
              {turns.map(turn => <Turn key={turn.id} turn={turn} targetName={turn.speakerType === 'human' ? participants.find(item => item.id === turn.participantId)?.roleName : undefined}/>) }
            ...
            <div className="discussion-composer">
              ...
              {mention && <div className="mention-popover discussion-mention"><div className="mention-title">选择要追问的角色</div>{mentionMatches.map(item => <button onMouseDown={event => { event.preventDefault(); chooseMention(item) }} key={item.id}><span className="agent-avatar">{roleIcon(item.roleName)}</span><div><strong>{item.roleName}</strong><small>{item.rolePrompt}</small></div><em>{item.isLeader ? 'Leader' : '定向追问'}</em></button>)}{!mentionMatches.length && <p>没有匹配的角色</p>}</div>}
...
function Turn({ turn, targetName }: { turn: ConversationTurn; targetName?: string }): import('react').JSX.Element {
  ...
  return <article className={`conversation-turn ${turn.speakerType} ${turn.status.toLowerCase()}`}><span className="turn-avatar">{turn.speakerType === 'human' ? 'Y' : turn.speakerType === 'system' ? 'S' : roleIcon(turn.speakerName)}</span><div><header><strong>{turn.speakerName}{targetName && <em> → @{targetName}</em>}</strong><small>{turn.status}{turn.inputTokens + turn.outputTokens > 0 ? ` · ${(turn.inputTokens + turn.outputTokens).toLocaleString()} tokens` : ''}</small></header>{body}</div></article>
}
