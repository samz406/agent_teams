...
describe("transactional local persistence", () => {
  it("persists changes, messages, artifacts and evidence lineage", () => {
      ...
      agentBindings: [
        {
          ...
          permissions: snapshot.agents[0].permissions,
        },
      ],
      tags: ["test"],
    });
    ...
    expect(
      restored.messages.some((message) => message.changeId === change.id),
    ).toBe(true);
  ...
  it("recovers an interrupted run, session, task and workstream as one transaction", () => {
    ...
    const change = db.createChange({
      title: "Recover",
      description: "Recover active domain state",
      workflowType: "bug-fix",
      priority: "P1",
      workspaceIds: [workspace.id],
      agentIds: [agent.id],
    ...
    const session = db.ensureAgentSession(
      change.id,
      agent.id,
      workspace.id,
      agent.runtime,
    ...
    expect(restored.runs.find((item) => item.id === run.id)?.status).toBe(
      "INTERRUPTED",
    );
    expect(restored.tasks.find((item) => item.id === task.id)?.status).toBe(
      "BLOCKED",
    );
    expect(
      restored.agentSessions.find((item) => item.id === session.id)?.status,
    ).toBe("INTERRUPTED");
    expect(
      restored.workstreams.find((item) => item.id === workstream.id)?.status,
    ).toBe("BLOCKED");
  });

  it("migrates existing conversations to sequenced incremental context without losing turns", () => {
    const directory = mkdtempSync(
      join(tmpdir(), "moxt-conversation-migration-"),
    );
    paths.push(directory);
    const databasePath = join(directory, "moxt.db");
    ...
    expect(restored.conversationParticipants[0]).toMatchObject({
      lastSeenTurnSequence: 1,
      sessionGeneration: 1,
    });
    ...
    const conversation = migrated.createConversation({
      title: "六帽分析",
      topic: "是否进入新市场",
      background: "",
      mode: "six-hats",
      maxRounds: 4,
      participants: hats.map((roleName, index) => ({
        agentId: executor.id,
        roleName,
        rolePrompt: roleName,
        isLeader: index === 0,
    ...
    expect(migrated.getConversationParticipants(conversation.id)).toHaveLength(
      6,
    );
  });
  ...
  it("converts legacy message or token stops to round-only completion", () => {
    const directory = mkdtempSync(
      join(tmpdir(), "moxt-round-limit-migration-"),
    );
    paths.push(directory);
    const databasePath = join(directory, "moxt.db");
    const legacy = new Sqlite(databasePath);
