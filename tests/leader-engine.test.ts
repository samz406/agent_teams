...
function setup(workflowType: WorkflowType = 'cross-project') {
  const directory = mkdtempSync(join(tmpdir(), 'agent-teams-leader-')); paths.push(directory)
  ...
  const change = db.createChange({ title: 'Change', description: 'Test evidence state machine', workflowType, priority: 'P1', workspaceIds: [workspace.id], agentIds: [agent.id], agentBindings: [{ agentId: agent.id, workspaceId: workspace.id, permissions: agent.permissions }], tags: [] })
...
describe('evidence-driven leader state machine', () => {
  ...
  it('does not stop a Bug Fix at the ON_LOOP Fix phase after evidence is accepted', () => {
    ...
    const run = createRun(change.id, agent.id, task.id); db.createRun(run); db.updateRun(run.id, { status: 'COMPLETED', exitCode: 0, finalResponse: 'Fixed and tested' })
    db.addEvidence(run.id, { type: 'RUNTIME', title: 'Runtime exit', status: 'PASS', detail: 'exit 0' })
    db.addEvidence(run.id, { type: 'DIFF', title: '1 file changed', status: 'UNVERIFIED', detail: 'diff' })
    db.addEvidence(run.id, { type: 'TEST', title: 'npm test', status: 'PASS', detail: 'passed' })
  ...
  it('treats REVIEW as an Agent review phase rather than a human stop', () => {
    const { db, agent, change, leader } = setup('bug-fix'); db.updateChangeState(change.id, 'RUNNING', 5)
    const current = db.getChange(change.id)!; const task = leader.createTask(current, agent, 'Review final diff and evidence')
    const run = createRun(change.id, agent.id, task.id); db.createRun(run); db.updateRun(run.id, { status: 'COMPLETED', exitCode: 0, finalResponse: 'Review passed' })
    db.addEvidence(run.id, { type: 'RUNTIME', title: 'Runtime exit', status: 'PASS', detail: 'exit 0' })
    expect(leader.onRunFinished(db.getRun(run.id)!).accepted).toBe(true)
