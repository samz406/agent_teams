...
import { ConversationEngine, type ConversationExecutor } from '../src/runtime/conversation-engine'
import type { Agent, Conversation, ConversationParticipant } from '../src/shared/contracts'
...
describe('conversation engine', () => {
  it('shares prior turns, respects round limit and ends ready to summarize', async () => {
    ...
    const restored = db.snapshot([]); const turns = restored.conversationTurns.filter(item => item.conversationId === conversation.id && item.speakerType !== 'system')
  ...
  it('prioritizes a directly addressed participant and creates a leader deliverable', async () => {
    const { db, conversation, executor, engine } = setup()
    const participants = db.getConversationParticipants(conversation.id); const target = participants.find(item => !item.isLeader)!
    engine.sendMessage(conversation.id, '请组织教练先分析我的反馈方式。', target.id)
    engine.start(conversation.id)
    await waitFor(() => db.getConversation(conversation.id)?.status === 'READY_TO_SUMMARIZE')
...
async function waitFor(predicate: () => boolean, timeout = 2000): Promise<void> {
  ...
  while (!predicate()) { if (Date.now() - started > timeout) throw new Error('Timed out'); await new Promise(resolve => setTimeout(resolve, 5)) }
}
