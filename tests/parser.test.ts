...
describe("real runtime output parser", () => {
  it("extracts native session identity and final response from JSONL", () => {
    const output = [
      ...
      JSON.stringify({
        final_response: "Root cause confirmed with test evidence.",
      }),
    ].join("\n");
  ...
  it("extracts exact usage when available and estimates when absent", () => {
    ...
    expect(extractTokenUsage("plain", "123456", "123")).toEqual({
      inputTokens: 2,
      outputTokens: 1,
    });
  });
  ...
  it("accepts only bounded, valid team delegation actions", () => {
    const actions = extractTeamActions(
...
\`\`\``);
    ...
    expect(actions[1]).toEqual({
      agent: "Architect",
      prompt: "Write integration notes",
    });
  });

  it("does not interpret unrelated json arrays as delegation", () => {
    expect(
      extractTeamActions('```json\n[{"name":"asset","path":"a.json"}]\n```'),
    ).toEqual([]);
  });
});

describe("work evidence parser", () => {
  it("only promotes valid fenced source and freshness declarations", () => {
    const value = extractWorkEvidence(
      '```work-evidence\n{"sources":[{"url":"https://example.com","retrievedAt":"2026-09-04T00:00:00Z"}],"dataFreshness":{"asOf":"2026-09-03T00:00:00Z","coverage":"24h"}}\n```',
    );
    expect(value.sources).toHaveLength(1);
    expect(value.dataFreshness?.coverage).toBe("24h");
  });
});
