...
describe("runtime usage metering", () => {
  ...
  it("keeps real Codex token usage when CLI omits model and does not invent a dollar cost", () => {
    const result = extractUsageSummary(
      JSON.stringify({
        type: "turn.completed",
        usage: {
          input_tokens: 9000,
  ...
  it("persists usage as evidence and aggregates multiple runs without inventing missing cost", () => {
    const first = extractUsageSummary(
      JSON.stringify({
        type: "turn.completed",
        model: "gpt-5.6-luna",
        usage: {
    ...
    const second = extractUsageSummary(
      JSON.stringify({
        type: "result",
        model: "claude-sonnet-5",
    ...
    expect(aggregate.usage.totalTokens).toBe(
    ...
    expect(parseUsageEvidence(runs[1].evidence[0].detail)?.costType).toBe(
      "REPORTED",
    );
  });

  it("marks a partially priced task cost as incomplete", () => {
    expect(formatCost({ costUsd: 0.42, pricedRuns: 2, usageRuns: 3 })).toBe(
      "~$0.42+",
    );
  });
});
...
function makeRun(
  id: string,
  summary: NonNullable<ReturnType<typeof extractUsageSummary>>,
): Run {
